...
function addFavorite(req, res, next) {
  ...
  'values(${id}, ${user_id}, ${title}, ${poster_path}, ${release_date}, ${vote_average}, ${overview}) returning id', req.body)
  .then(function(data) {
    res.status(200).json({ status: 'success', message: "Movie was added to favorites");
  }).catch(function(err) {
    next(err);
  })
}
